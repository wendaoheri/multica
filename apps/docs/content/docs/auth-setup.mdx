...
The code must be 6 digits. The fixed code is double-gated: it only works when `APP_ENV` is non-production **and** `MULTICA_DEV_VERIFICATION_CODE_ENABLED=true`.

<Callout type="warning">
Do not enable a fixed code on a publicly reachable instance. The production combination is `APP_ENV=production` with an empty `MULTICA_DEV_VERIFICATION_CODE` (or `MULTICA_DEV_VERIFICATION_CODE_ENABLED` unset).
</Callout>
