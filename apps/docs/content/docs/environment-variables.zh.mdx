...
| `MULTICA_DEV_VERIFICATION_CODE` | 空 | 非 production 环境使用的固定 6 位测试验证码；双重门控，需同时满足非 production 的 `APP_ENV` 和 `MULTICA_DEV_VERIFICATION_CODE_ENABLED=true` |
| `MULTICA_DEV_VERIFICATION_CODE_ENABLED` | 空 | 固定测试验证码的显式开关；仅 `true` 或 `1` 生效 |
