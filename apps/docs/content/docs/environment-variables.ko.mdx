...
| `MULTICA_DEV_VERIFICATION_CODE` | 비어 있음 | production이 아닌 환경에서 사용하는 고정 6자리 테스트 인증 코드. 이중 게이트: non-production `APP_ENV`와 `MULTICA_DEV_VERIFICATION_CODE_ENABLED=true`가 모두 필요 |
| `MULTICA_DEV_VERIFICATION_CODE_ENABLED` | 비어 있음 | 고정 테스트 인증 코드의 명시적 스위치. `true` 또는 `1`만 활성화 |
