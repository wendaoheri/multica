...
> **Note:** If neither Resend nor SMTP is configured, generated verification codes are printed to backend logs — copy them from there to log in. A fixed local testing code (e.g. `888888`) is **opt-in only** and double-gated: set `MULTICA_DEV_VERIFICATION_CODE=888888` **and** `MULTICA_DEV_VERIFICATION_CODE_ENABLED=true` in `.env`, and keep `APP_ENV` non-production. The Docker self-host stack pins `APP_ENV=production`, so the shortcut is ignored there even if both variables are set. **Never enable a fixed code on a publicly reachable instance.**
